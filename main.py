...
def clear_flags(message, callback=False, not_delete=()):
    ...
    flags = (
             'msg_text.dev_bots.flag_develop_bots',
             ...
             'msg_text.design_obj.send_doc', 'msg_text.site.send_doc',
             'msg_text.blog.flag_for_bloggers',
             'msg_text.blog.flag_detail_product', 'msg_text.blog.flag_aim',
             'msg_text.blog.flag_budget', 'msg_text.admin.flag_for_password',
             'msg_text.prom_tg.msg_for_delete',
             'msg_text.prom_tg.flag_prom_tg', 'msg_text.prom_tg.newsletter',
             'msg_text.prom_tg.number_newsletter',
             'msg_text.prom_tg.complex_href',
             'msg_text.prom_tg.complex_contact',
             'msg_text.reg_user.dialog_support',
             'msg_text.admin.flag_for_private_msg'
             )
    for flag in flags:
...
async def send_msg(
        message, text_user, text_admin=None, admins=('qzark', 'zakazy'),
        markup=None):
    clear_flags(message)
    if markup:
        await bot.send_message(chat_id=message.chat.id, text=text_user,
                               reply_markup=markup, parse_mode='html')
    else:
        await bot.send_message(chat_id=message.chat.id, text=text_user,
                               parse_mode='html')
    text_admin = (
        f'Юзернейм: @{models.db_object.db_select_user(message)}\n{text_admin}'
        )
    for admin in admins:
        await bot.send_message(chat_id=settings.ADMINS[admin],
                               text=text_admin, parse_mode='html')


async def services(message):
    text = msg_text.base.services()
    await bot.send_message(chat_id=message.chat.id, text=text,
                           reply_markup=markups.services_markup)
...
async def administration(message):
    await send_msg(message, text_user=msg_text.admin.write_password(),
                   admins=())
...
async def get_messages(message):
    ...
    if (
        msg_text.admin.flag_account.get(message.chat.id) and
        message.text == 'Рассылки сообщений'
    ):
    ...
    # Sending out messages to all users
        ...
        msg_text.admin.flag_for_newsletter.get(message.chat.id) and
        message.text != 'Стоп'
    ):
    ...
    # Base to send a user to whom to send message
        ...
        msg_text.admin.flag_account.get(message.chat.id) and
        message.text == 'Личное сообщение от бота'
    ):
        clear_admin_flags(message)
    ...
    # Getting the user to send private message
        ...
        msg_text.admin.flag_account.get(message.chat.id) and
        msg_text.admin.flag_for_write_user.get(message.chat.id)
    ):
        clear_admin_flags(message)
        ...
        try:
            user_id = models.db_object.db_select_user_id(
                msg_text.admin.user_to_send[message.chat.id][1:])
        except Exception:
            user_id = '-'
        msg_text.reg_user.dialog_support[user_id] = True

        await bot.send_message(chat_id=message.chat.id, text=text,
                               reply_markup=markups.close_the_dialog_markup)

    # Sending private message to user
    elif (
        msg_text.admin.flag_for_private_msg.get(message.chat.id) and
        msg_text.admin.flag_account.get(message.chat.id)
    ):
        if message.text != 'Закрыть диалог':
            ...
            user_id = models.db_object.db_select_user_id(
                msg_text.admin.user_to_send[message.chat.id][1:])
            await bot.send_message(chat_id=message.chat.id, text=text_admin,
                                   reply_markup=markups.admin_markup)
            await bot.send_message(chat_id=user_id, text=text_admin,
                                   reply_markup=markups.main_markup)
            clear_admin_flags(message)
            try:
    ...
    # Base to see all users subscribed to bot
        ...
        msg_text.admin.flag_account.get(message.chat.id) and
        message.text == 'Просмотр всех пользователей'
    ):
    ...
    # The dialog with the support
        ...
        if message.text != 'Закрыть диалог':
            text = message.text
            await bot.send_message(
                chat_id=settings.ADMINS['qzark'],
                text=text,
                reply_markup=markups.close_the_dialog_markup,
                parse_mode='html')
            # await bot.send_message(
            # chat_id=settings.ADMINS['zakazy'],
            # text=text,
            # reply_markup=markups.close_the_dialog_markup,
            # parse_mode='html')
        else:
    ...
    # Base to see variants of payments
        ...
        await bot.send_message(chat_id=message.chat.id, text=text,
                               reply_markup=markups.href_service_markup)
    ...
    # Processing incoming messages and sending them to admins
        ...
        if message.text != 'Далее':
            ...
            text_admin = msg_text.base.category[message.chat.id]
            await send_msg(message=message,
                           text_user=msg_text.dev_bots.finish(),
    ...
    # The answer from the first question
        ...
        await send_msg(message, text_user=msg_text.blog.detail_product(),
                       admins=(), markup=markups.continue_markup)
    ...
    # The answer from the second question
        ...
        if message.text != 'Далее':
            if (
                'Расскажите подробнее, о вашем товаре/услуге?:' in
                msg_text.base.category[message.chat.id]
            ):
            ...
            if (
                'Расскажите подробнее, о вашем товаре/услуге?:'
                not in msg_text.base.category[message.chat.id]
            ):
                msg_text.base.category[message.chat.id] += (
                    '\nРасскажите подробнее, о вашем товаре/услуге?: ' + '-'
                    )
    ...
    # The answer from the third question
        ...
        if message.text != 'Далее':
            if (
                ...
                in msg_text.base.category[message.chat.id]
            ):
                msg_text.base.category[message.chat.id] += '   ' + message.text
            else:
                msg_text.base.category[message.chat.id] += (
            ...
            await send_msg(message, text_user=msg_text.blog.budget(),
                           admins=(), markup=markups.clean_markup)
    ...
    # The answer from the fourth question
        ...
        await send_msg(message, text_user=msg_text.blog.finish(),
                       text_admin=msg_text.base.category[message.chat.id],
                       markup=markups.main_markup)
    ...
    # Base to go over to main menu of promotion telegram
    elif message.text == '<< Назад' and msg_text.prom_tg.flag_prom_tg.get(
        message.chat.id
    ):
        ...
        await bot.delete_message(
            ...
            message_id=msg_text.prom_tg.msg_for_delete[message.chat.id].id)
        await bot.send_message(chat_id=message.chat.id, text=text,
    ...
    # PR-company
        ...
        msg_text.prom_tg.flag_prom_tg.get(message.chat.id) and
        message.text != '<< Назад'
    ):
        if message.text != 'Далее':
            msg_text.base.category[message.chat.id] += f'\n{message.text}'
            ...
            await send_msg(
                message,
                text_user=msg_text.ListPromotionTelegramInvite().finish(),
                text_admin=text_admin, markup=markups.main_markup)
    ...
    # Newsletter in Telegram
        ...
        if message.text != 'Далее':
            ...
            await send_msg(message, text_user=text, admins=(),
                           markup=markups.prom_tg_newsletter_number_markup)
    ...
    # Complex to get contact from promotion telegram
        ...
        text_admin = msg_text.base.category[message.chat.id]
        await send_msg(message, text_user=text, text_admin=text_admin,
                       markup=markups.main_markup)
    ...
    # Processing brief
        ...
        msg_text.site.send_doc.get(message.chat.id) or
        msg_text.design_obj.send_doc.get(message.chat.id)
    ):
        text_admin = (
            msg_text.base.category.get(message.chat.id) + '\n' + message.text
            )
        await send_msg(message=message, text_user=msg_text.site.finish(),
                       text_admin=text_admin)

    # Appeal to support
    elif (
        msg_text.design_obj.flag_sup_brief.get(message.chat.id) or
        msg_text.site.flag_sup_brief.get(message.chat.id)
    ):
        text_admin = (
            msg_text.base.category.get(message.chat.id) + '\n' + message.text
            )
    ...
    # Unknown message
        ...
        await send_msg(message=message, text_user=msg_text.base.unknown(),
                       admins=())
...
async def newsletter(message):
    if msg_text.admin.flag_for_newsletter.get(message.chat.id):
        ...
        for chat_id in users_id:
            if message.sticker:
                await bot.send_sticker(chat_id=chat_id[0],
                                       sticker=message.sticker.file_id)
            if message.photo:
                await bot.send_photo(chat_id=chat_id[0],
                                     photo=message.json['photo'][0]['file_id'])
            if message.location and not message.venue:
                await bot.send_location(chat_id=chat_id[0],
                                        longitude=message.location.longitude,
                                        latitude=message.location.latitude)
            if message.venue:
                await bot.send_venue(chat_id=chat_id[0],
                                     longitude=message.location.longitude,
                                     latitude=message.location.latitude,
                                     title=message.venue.title,
                                     address=message.venue.address)
            if message.video:
                await bot.send_video(chat_id=chat_id[0],
                                     video=message.video.file_id,
                                     )
            if message.voice:
                await bot.send_voice(chat_id=chat_id[0],
...
async def get_docs(message):
    if (
        msg_text.site.flag_sites.get(message.chat.id) or
        msg_text.design_obj.flag_design.get(message.chat.id)
    ):
        ...
        await send_msg(message, text_user=text_user, text_admin=text_category,
                       markup=markups.main_markup)
        await bot.send_document(chat_id=settings.ADMINS['qzark'],
                                document=message.document.file_id)  # qzark
        await bot.send_document(chat_id=settings.ADMINS['zakazy'],
                                document=message.document.file_id)  # zakazy

    elif msg_text.admin.flag_for_newsletter.get(message.chat.id):
        users_id = models.db_object.db_select_all_users_id()
        for chat_id in users_id:
            if message.document:
                await bot.send_document(chat_id=chat_id[0],
...
@bot.callback_query_handler(
        func=lambda callback: callback.data == 'develop_bots')
async def develop_bots(callback):
    msg_text.base.category[callback.message.chat.id] = (
        '<strong>Разработка чат-ботов</strong>'
        )
...
async def bloggers(callback):
    msg_text.base.category[callback.message.chat.id] = (
        '<strong>Реклама у блогеров</strong>'
        )
...
@bot.callback_query_handler(
        func=lambda callback: callback.data == 'promotion_telegram')
async def prom_telegram(callback):
    msg_text.base.category[callback.message.chat.id] = (
        '<strong>Продвижение в телеграмм</strong>'
        )
    text = msg_text.prom_tg.start()
...
async def prom_tg_newsletter(callback):
    ...
    await general_prom_tg(callback, text_start=text,
                          markup=markups.prom_tg_newsletter_markup)


@bot.callback_query_handler(
        func=lambda callback: callback.data == 'prom_tg_1_want')
...
@bot.callback_query_handler(
        func=lambda callback: 'prom_tg_1_number' in callback.data)
async def prom_tg_newsletter_number(callback):
    ...
    if callback.data.split('_')[-1] == '6' and 'inp' not in callback.data:
        text = (
            msg_text.ListPromotionTelegramNewsletter().number() + '\n'
            + 'Количество: '
            )
        ...
        msg_text.prom_tg.number_newsletter[callback.message.chat.id] = (
            msg_text.prom_tg.number_newsletter.get(
        ...
        await send_msg(callback.message, text_user=text_user,
                       text_admin=text_admin, markup=markups.main_markup)


@bot.callback_query_handler(
        func=lambda callback: callback.data == 'prom_tg_1_sup')
async def prom_tg_newsletter_support(callback):
    await bot.delete_message(chat_id=callback.message.chat.id,
                             message_id=callback.message.id)
    msg_text.base.category[callback.message.chat.id] += '\nНужна ваша помощь'
    text = msg_text.ListPromotionTelegramNewsletter().finish()
    await send_msg(callback.message, text_user=text,
                   text_admin=msg_text.base.category[callback.message.chat.id])
...
async def prom_tg_invite(callback):
    ...
    await general_prom_tg(callback, text_start=text,
                          markup=markups.prom_tg_invite_markup)


@bot.callback_query_handler(
        func=lambda callback: callback.data == 'prom_tg_2_want')
...
async def prom_tg_parsing(callback):
    ...
    await general_prom_tg(callback, text_start=text,
                          markup=markups.back_continue_markup)
...
@bot.callback_query_handler(
        func=lambda callback: callback.data in ['prom_tg_5', 'prom_tg_6'])
async def prom_tg_cycle_comments(callback):
    ...
    await general_prom_tg(callback, text_start=text,
                          markup=markups.prom_tg_cycle_comments_markup)


@bot.callback_query_handler(
        func=lambda callback: callback.data == 'prom_tg_5_6_want')
...
async def prom_tg_complex(callback):
    ...
    await general_prom_tg(callback, text_start=text,
                          markup=markups.prom_tg_complex_markup)


@bot.callback_query_handler(
        func=lambda callback: callback.data == 'prom_tg_7_want')
...
async def general_prom_tg(callback, text_start, markup=None):
    msg_text.prom_tg.category[callback.message.chat.id] = (
        msg_text.prom_tg.all_categories[callback.data.split('_')[-1]]
        )
    ...
    if markup:
        ...
        msg = await bot.send_message(chat_id=callback.message.chat.id,
                                     text=text_start, parse_mode='html')
...
async def sites(callback):
    ...
    await bot.edit_message_text(chat_id=callback.message.chat.id,
                                message_id=callback.message.id,
                                text=text,
                                reply_markup=markups.brief_site_markup)
...
async def design(callback):
    ...
    await bot.edit_message_text(chat_id=callback.message.chat.id,
                                message_id=callback.message.id,
                                text=text,
                                reply_markup=markups.brief_design_markup)


@bot.callback_query_handler(
        func=lambda callback: callback.data == 'back_service')
